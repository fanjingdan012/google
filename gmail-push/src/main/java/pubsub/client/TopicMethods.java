...
public final class TopicMethods {
     ...
     */
    public static void createTopic(final Pubsub client, final String[] args) throws IOException {
        // Main.checkArgsLength(args, 3);
        String topicName = PubsubUtils.getFullyQualifiedResourceName(PubsubUtils.ResourceType.TOPIC, args[0], args[2]);
     ...
     *             when Cloud Pub/Sub API calls fail.
     */
    public static void connectIrc(final Pubsub client, final String[] args) throws IOException {
        // Main.checkArgsLength(args, 5);
        final String server = args[3];
        final String channel = args[4];
        final String topic = PubsubUtils
                .getFullyQualifiedResourceName(PubsubUtils.ResourceType.TOPIC, args[0], args[2]);
        ...
        Pattern pat = Pattern.compile("\\u000314\\[\\[\\u000307(.*)\\u000314\\]\\]\\u0003.*"
                + "\\u000302(http://[^\\u0003]*)\\u0003");
        while ((line = reader.readLine()) != null) {
            if (line.toLowerCase().startsWith("PING ")) {
                ...
                line = line.substring(prividx + privmsgMark.length(), line.length());
                PubsubMessage pubsubMessage = new PubsubMessage();
                Matcher matcher = pat.matcher(line);
                if (matcher.find()) {
                    String message = String.format("Title: %s, Diff: %s", matcher.group(1), matcher.group(2));
     ...
     *             when Cloud Pub/Sub API calls fail.
     */
    public static void publishMessage(final Pubsub client, final String[] args) throws IOException {
        // Main.checkArgsLength(args, 4);
        String topic = PubsubUtils.getFullyQualifiedResourceName(PubsubUtils.ResourceType.TOPIC, args[0], args[2]);
     ...
     *             when Cloud Pub/Sub API calls fail.
     */
    public static void deleteTopic(final Pubsub client, final String[] args) throws IOException {
        // Main.checkArgsLength(args, 3);
        String topicName = PubsubUtils.getFullyQualifiedResourceName(PubsubUtils.ResourceType.TOPIC, args[0], args[2]);
     ...
     *             when Cloud Pub/Sub API calls fail.
     */
    public static void listTopics(final Pubsub client, final String[] args) throws IOException {
        String nextPageToken = null;
        boolean hasTopics = false;
        Pubsub.Projects.Topics.List listMethod = client.projects().topics().list("projects/" + args[0]);
        do {
        ...
        if (!hasTopics) {
            System.out.println(String.format("There are no topics in the project '%s'.", args[0]));
        }
    }
}
