...
public final class SubscriptionMethods {
     ...
     * @param args
     *            Arguments as an array of String.
     * @throws IOException
     *             when Cloud Pub/Sub API calls fail.
     */
    public static void createSubscription(final Pubsub client, final String[] args) throws IOException {
        // Main.checkArgsLength(args, 4);
        String subscriptionName = PubsubUtils.getFullyQualifiedResourceName(PubsubUtils.ResourceType.SUBSCRIPTION,
                args[0], args[2]);
        Subscription subscription = new Subscription().setTopic(PubsubUtils.getFullyQualifiedResourceName(
     ...
     *             when Cloud Pub/Sub API calls fail.
     */
    public static void pullMessages(final Pubsub client, final String[] args) throws IOException {
     ...
     *             when Cloud Pub/Sub API calls fail.
     */
    public static void listSubscriptions(final Pubsub client, final String[] args) throws IOException {
        String nextPageToken = null;
        boolean hasSubscriptions = false;
        Pubsub.Projects.Subscriptions.List listMethod = client.projects().subscriptions().list("projects/" + args[0]);
        do {
        ...
        if (!hasSubscriptions) {
            System.out.println(String.format("There are no subscriptions in the project '%s'.", args[0]));
        }
    }
     ...
     *             when Cloud Pub/Sub API calls fail.
     */
    public static void deleteSubscription(final Pubsub client, final String[] args) throws IOException {
        // Main.checkArgsLength(args, 3);
        String subscriptionName = PubsubUtils.getFullyQualifiedResourceName(PubsubUtils.ResourceType.SUBSCRIPTION,
                args[0], args[2]);
